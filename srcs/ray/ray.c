...
/*   Updated: 2025/11/12 14:56:35 by mlouis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	hit_plane(t_ray r, t_lst *pls)
{
	t_plane	closest_pl;
	double	denom;
	double	t;
	...
	while (*pls)
	{
		denom = vect3_mult(pls->content.normal, r.dir);
		if (fabs(denom) < DBL_EPSILON)
			continue ;
		closest_t = t;
		t = vect3_mult(vect3_sub(pls->content.origin, r.origin), pls->content.normal) / denom;
		if (t > 0 && t < closest_t)
		{
			closest_t = t;
			closest_pl = pls->content;
			hit = true;
		}
		*pls = pls->next;
	}
...
void	calculate_t_axis(double ***t, t_ray ray, t_box box)
{
