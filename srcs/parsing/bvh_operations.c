...
int	bvh_add(t_bvh **ptr_root, t_bvh *node)
{
	...
	if (*ptr_root != NULL)
	{
		if ((*ptr_root)->obj.type != NONE
			|| box_contained((*ptr_root)->box, node->box) == FALSE)
		...
		if (ret != SUCCESS)
			return (ret);
		(*ptr_root)->nb_leaves = (*ptr_root)->left->nb_leaves
			+ (*ptr_root)->right->nb_leaves;
		return (SUCCESS);
	}
	*ptr_root = node;
	node->nb_leaves = 1;
...
t_bvh	*find_insert(t_bvh *root, t_bvh *node, double *ptr_cost)
{
	...
	{
		if (left_path->obj.type == NONE
			&& box_contained(left_path->box, node->box) == TRUE)
			left_path = find_insert(left_path, node, &left_cost);
		if (left_path->obj.type == NONE
			&& box_contained(right_path->box, node->box) == TRUE)
